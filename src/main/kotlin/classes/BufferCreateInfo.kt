...
class BufferCreateInfo(
    ...
    val MemoryStack.native: Ptr
        get() = ncalloc(VkDeviceQueueCreateInfo.ALIGNOF, 1, VkDeviceQueueCreateInfo.SIZEOF).also { toPtr(it) }

    infix fun MemoryStack.toPtr(ptr: Ptr) {
        nsType(ptr, type.i)
        npNext(ptr, next)
        nflags(ptr, flags)
        nsize(ptr, this@BufferCreateInfo.size.L)
        nusage(ptr, usageFlags)
        nsharingMode(ptr, sharingMode.i)
        npQueueFamilyIndices(ptr, queueFamilyIndices?.toIntBuffer(this))
    }
}