...
class CommandBufferInheritanceInfo(
    ...
    val MemoryStack.native: Ptr
        get() = ncalloc(ALIGNOF, 1, SIZEOF).also {
            nsType(it, type.i)
            npNext(it, next)
            nrenderPass(it, renderPass.L)
            nframebuffer(it, framebuffer.L)
            nocclusionQueryEnable(it, occlusionQueryEnable.i)
            nqueryFlags(it, queryFlags)
            npipelineStatistics(it, pipelineStatistics)
        }
}